...
Before a controller can connect to any devices, it must first try to discover any matching devices on the local network. The QUERY message contains two data fields: the type of device the controller is searching for, light sensor, temperature sensor, informational display or otherwise, and the name of the device sending the QUERY message. This packet is sent as a broadcast transmission to all devices on the network.
\label{ssub:query}
\begin{center}
\begin{bytefield}{32}
\bitheader{0,8,16,24,31}\\
...
After a device receives a QUERY message, it can decide whether or not to reply to it, if so it replies with the QUERY response message. The QUERY Response message contains 3 data fields: the type and name of the device which is responding and the rate, which either indicates the default rate that a sensor transmits readings at, or the rate at which an actuator will expect PING messages from a connected controller to confirm the connection is still alive.
...
Once a device has been located, either a priori or through querying, a controller needs to send a CONNECT message to start the process of forming a connection with the desired device. As shown in the message sequence diagrams, this uses a three-way handshake, of which this is the initial message. The CONNECT message contains 2 data fields: the rate at which the controller either wants to receive data readings from sensors, or the rate at which it will PING actuators, and the name of the controller.
\label{ssub:connect}
\begin{center}
\begin{bytefield}{32}
\bitheader{0,8,16,24,31}\\
...
Once a device receives a CONNECT message, unlike when receiving a QUERY message, it must respond with the second part of the three-way handshake with either a positive or negative acknowledgement.
