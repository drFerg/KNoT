...
\begin{itemize}
	\item Minimalistic protocol, reliability for critical data only
...
\begin{itemize}
	...
	\item a payload length field which contains the size of the payload,
...
java jms centralised, good to reliably find devices, costly service, if it fails network fails, yet all other devices are active
not a good fit with distributed mesh nature of IoT

\subsubsection{Cloud vs Local} % (fold)
\label{ssub:cloud_vs_local}
SmartThings taking cloud first approach, wiring all devices in the cloud, provides user access from anywhere instantly but poses risks to security and creates a single point of failure, not unlikely(list failures of amazon and other cloud), what happens then? knot approach, wire locally and control from anywhere through appropriate abstractions and applications
...
xAP, fully multicast network, everything broadcasts, every device picks up messages, allows for easy adding on to network, but creates high cost for every device on the network, especially as traffic increases. unscalable
...
xAP, use of textual encoding for packet header and payload ala JSON, costly to transport, inefficient store of data, easy to read but how often will user need to read un-processed data
...
Twine, idea of internet of thing, connecting device by itself and then connecting to other services, limited functionality. instead create a local internet of things which link up, providing much richer local experience with possibility to utilise online services etc.
